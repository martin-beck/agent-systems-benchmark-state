# Agent Systems Benchmark live project state

Generated from local Git and GitHub. Do not edit.

- Product remote main: `0fbec95d2154cfa3efd522a0a49e98d492b77a22`
- Local origin/main: `0fbec95d2154cfa3efd522a0a49e98d492b77a22`
- Primary worktree head: `096dc4f275c05ad81772f443b6f22dddfb92da3d`

## Open pull requests

| PR | Head | Base | Merge | Checks | Title |
| --- | --- | --- | --- | --- | --- |
| #31 | `feature/native-platforms@bdaf9c108226` | `main` | UNKNOWN | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS | platforms: add fail-closed native qualification evidence |
| #106 | `feature/one-line-local-install@c6b76fa76519` | `main` | UNKNOWN | IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS:, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS:, IN_PROGRESS: | feat(install): add verified rootless bootstrap |

## Recent workflows

| Run | SHA | Event | Workflow | State |
| --- | --- | --- | --- | --- |
| 34320597248 | `0fbec95d2154` | push | Huawei MIT source headers | in_progress:- |
| 34320597244 | `0fbec95d2154` | push | Emulated aarch64 portability | pending:- |
| 34320597235 | `0fbec95d2154` | push | Rust verification | queued:- |
| 34320597223 | `0fbec95d2154` | push | Repository quality | in_progress:- |
| 34320597218 | `0fbec95d2154` | push | Formal assurance | queued:- |
| 34320597215 | `0fbec95d2154` | push | Fault assurance | queued:- |
| 34320570236 | `c6b76fa76519` | pull_request | Repository quality | in_progress:- |
| 34320570228 | `c6b76fa76519` | pull_request | Rust verification | in_progress:- |
| 34320570216 | `c6b76fa76519` | pull_request | Agent Workflow Quality shadow | completed:success |
| 34320570186 | `c6b76fa76519` | pull_request | Formal assurance | in_progress:- |
| 34320570172 | `c6b76fa76519` | pull_request | Huawei MIT source headers | completed:success |
| 34320570154 | `c6b76fa76519` | pull_request | Emulated aarch64 portability | in_progress:- |
