# Agent Systems Benchmark live project state

Generated from local Git and GitHub. Do not edit.

- Product remote main: `b2707c482876dcfb42c756c39165f6ecdb5c7c10`
- Local origin/main: `b2707c482876dcfb42c756c39165f6ecdb5c7c10`
- Primary worktree head: `b2707c482876dcfb42c756c39165f6ecdb5c7c10`

## Open pull requests

| PR | Head | Base | Merge | Checks | Title |
| --- | --- | --- | --- | --- | --- |
| #31 | `feature/native-platforms@bdaf9c108226` | `main` | DIRTY | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS | platforms: add fail-closed native qualification evidence |
| #61 | `feature/kernel-diagnostics@1d24993771a4` | `main` | CLEAN | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS | metrics: add fail-closed optional kernel probes |
| #82 | `fix/huawei-mit-license-headers@f2e54a0539e8` | `main` | UNSTABLE | COMPLETED:FAILURE, COMPLETED:FAILURE, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS | chore: enforce Huawei MIT source headers |

## Recent workflows

| Run | SHA | Event | Workflow | State |
| --- | --- | --- | --- | --- |
| 34249416603 | `b2707c482876` | push | Repository quality | in_progress:- |
| 34249416597 | `b2707c482876` | push | Emulated aarch64 portability | in_progress:- |
| 34249416594 | `b2707c482876` | push | Formal assurance | completed:success |
| 34249416581 | `b2707c482876` | push | Rust verification | completed:success |
| 34249416580 | `b2707c482876` | push | Fault assurance | completed:success |
| 34249065386 | `f2e54a0539e8` | pull_request | Fault assurance | completed:success |
| 34249065339 | `f2e54a0539e8` | pull_request | Huawei MIT source headers | completed:failure |
| 34249065331 | `f2e54a0539e8` | pull_request | Repository quality | completed:failure |
| 34249065320 | `f2e54a0539e8` | pull_request | Emulated aarch64 portability | completed:success |
| 34249065307 | `f2e54a0539e8` | pull_request | Formal assurance | completed:success |
| 34249065301 | `f2e54a0539e8` | pull_request | Rust verification | completed:success |
| 34249065299 | `f2e54a0539e8` | pull_request | Agent Workflow Quality shadow | completed:success |
