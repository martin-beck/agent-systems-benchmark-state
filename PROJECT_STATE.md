# Agent Systems Benchmark live project state

Generated from local Git and GitHub. Do not edit.

- Product remote main: `9db4b6d74d4442354f8bf29a46f3ab38f38f36db`
- Local origin/main: `9db4b6d74d4442354f8bf29a46f3ab38f38f36db`
- Primary worktree head: `9db4b6d74d4442354f8bf29a46f3ab38f38f36db`

## Open pull requests

| PR | Head | Base | Merge | Checks | Title |
| --- | --- | --- | --- | --- | --- |
| #5 | `feature/platform-manifests@dff27a9acb4f` | `main` | CLEAN | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SUCCESS | platforms: pin initial support manifests |
| #6 | `feature/durable-results@f6cd02eef6b7` | `main` | UNSTABLE | IN_PROGRESS:, IN_PROGRESS:, QUEUED: | Add crash-consistent durable result storage |

## Recent workflows

| Run | SHA | Event | Workflow | State |
| --- | --- | --- | --- | --- |
| 34047020883 | `f6cd02eef6b7` | pull_request | Repository quality | in_progress:- |
| 34047020851 | `f6cd02eef6b7` | pull_request | Rust verification | in_progress:- |
| 34046717411 | `dff27a9acb4f` | pull_request | Rust verification | completed:success |
| 34046717386 | `dff27a9acb4f` | pull_request | Repository quality | completed:success |
| 34046060074 | `9db4b6d74d44` | push | Rust verification | completed:success |
| 34046060062 | `9db4b6d74d44` | push | Repository quality | completed:success |
| 34045836000 | `3d04a2d5a3b3` | pull_request | Rust verification | completed:success |
| 34045835936 | `3d04a2d5a3b3` | pull_request | Repository quality | completed:success |
| 34045704998 | `3baa4f9d0a74` | push | Repository quality | completed:success |
| 34045704995 | `3baa4f9d0a74` | push | Rust verification | completed:success |
| 34045687393 | `2114472eb20c` | pull_request | Rust verification | completed:success |
| 34045687392 | `2114472eb20c` | pull_request | Repository quality | completed:success |
