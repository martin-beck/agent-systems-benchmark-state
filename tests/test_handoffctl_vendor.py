...
CURRENT_UPSTREAM_COMMIT = "2666c9d7a56ce02340e1b4b09344a227936a34cf"
CURRENT_MANIFEST_SHA256 = "f42db956e0baf8e91af957715f459ced9afe692c177e154f16cda57f299ae436"
ATTESTED_IDENTITIES = {
...
class VendorTest(unittest.TestCase):
    ...
    def test_additive_merge_attestation_is_exact_and_honest(self) -> None:
        ...
        self.assertEqual("v0.3.14", manifest["upstream"]["version"])
    ...
    def test_sync_verify_and_detect_tampering(self) -> None:
        ...
        with patch("builtins.print") as output:
            VENDOR.sync(self.source, self.target, "v0.3.14", commit)
    ...
    def test_release_identity_requires_clean_exact_tag(self) -> None:
        with patch.object(VENDOR, "git_output", side_effect=["", "b" * 40, "v0.3.14"]):
            self.assertEqual("b" * 40, VENDOR.release_identity(self.source, "v0.3.14"))
        with (
            patch.object(VENDOR, "git_output", return_value="dirty"),
            self.assertRaisesRegex(RuntimeError, "must be clean"),
        ):
            VENDOR.release_identity(self.source, "v0.3.14")
        with (
    ...
    def test_verify_rejects_identity_manifest_and_runtime_mismatch(self) -> None:
        ...
        core.write_text(
            core.read_text().replace(
                'COORDINATOR_VERSION = "0.3.14"', 'COORDINATOR_VERSION = "9.9.9"'
            )
        )
        ...
        with (
            patch.object(VENDOR, "git_output", side_effect=["", "short", "v0.3.14"]),
    ...
    def test_main_dispatches_sync_and_verify(self) -> None:
        ...
        with (
            ...
            self.assertEqual(0, VENDOR.main())
            sync.assert_called_once_with(self.source, self.target, "v0.3.14", "c" * 40)
