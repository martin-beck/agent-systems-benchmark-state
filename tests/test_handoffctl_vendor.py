...
CURRENT_UPSTREAM_COMMIT = "a1bc4459f884ce447e8ee2884df12ea3ff4b710b"
CURRENT_MANIFEST_SHA256 = "195d90e2a540fac40f5966635e87b6631a267c5bbffc43eafd9427d9e0a6077c"
ATTESTED_IDENTITIES = {
...
class VendorTest(unittest.TestCase):
    ...
    def test_additive_merge_attestation_is_exact_and_honest(self) -> None:
        ...
        self.assertEqual("v0.3.6", manifest["upstream"]["version"])
    ...
    def test_sync_verify_and_detect_tampering(self) -> None:
        ...
        with patch("builtins.print") as output:
            VENDOR.sync(self.source, self.target, "v0.3.6", commit)
    ...
    def test_release_identity_requires_clean_exact_tag(self) -> None:
        with patch.object(VENDOR, "git_output", side_effect=["", "b" * 40, "v0.3.6"]):
            self.assertEqual("b" * 40, VENDOR.release_identity(self.source, "v0.3.6"))
        with (
            patch.object(VENDOR, "git_output", return_value="dirty"),
            self.assertRaisesRegex(RuntimeError, "must be clean"),
        ):
            VENDOR.release_identity(self.source, "v0.3.6")
        with (
    ...
    def test_verify_rejects_identity_manifest_and_runtime_mismatch(self) -> None:
        ...
        core.write_text(
            core.read_text().replace(
                'COORDINATOR_VERSION = "0.3.6"', 'COORDINATOR_VERSION = "9.9.9"'
            )
        )
        ...
        with (
            patch.object(VENDOR, "git_output", side_effect=["", "short", "v0.3.6"]),
    ...
    def test_main_dispatches_sync_and_verify(self) -> None:
        ...
        with (
            ...
            self.assertEqual(0, VENDOR.main())
            sync.assert_called_once_with(self.source, self.target, "v0.3.6", "c" * 40)
