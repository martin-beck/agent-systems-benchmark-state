...
flowchart LR
    ...
    subgraph series_12["12 - Additional work"]
        ...
        AR_1212["AR-1212 - In progress"]:::status_in_progress
...
| P0 | [AR-1324](../tasks/AR-1324-auth-helper-control-contract.md): Authenticated credential-helper control contract | codex | Own the ASB control and runner backend for safe credential-helper invocation. | Monitor authoritative pull_request checks for PR #248 at 0116bf1; merge only after all required checks pass, then perform ASB↔asb-tui first-user/live-provider wizard acceptance. |
...
| P1 | [AR-1325](../tasks/AR-1325-provider-openrouter.md): Support a shared OpenRouter provider | Unclaimed | Define one pinned credential-free OpenRouter provider profile for compatible ASB agent adapters. | Pin the OpenRouter endpoint identity, dated model snapshot, transport bounds and OPENROUTER_API_KEY credential reference, then implement the credential-free provider profile with fail-closed validation. |
