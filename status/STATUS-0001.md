...
| P1 | [AR-1350](../tasks/AR-1350-sandbox-credential-channel.md): Sandbox-owned credential channel | codex-asb-ar1350-sandbox-channel-luna56 | Implement a sandbox-owned sealed-FD credential channel for live provider children. | Final channel design: runtime creates it only from validated LiveProviderRuntimeConfig (private constructor), adapter capabilities can only consume that channel, and credential bytes are transferred by owned Vec then erased. Bubblewrap receives a sealed memfd via --args FD and sets the bound target variable inside the child; secret is absent from argv/evidence. Focused 3/3, runtime clippy, workspace clippy, workspace test (188+ passed with documented ignores), and formatting have passed. Run final docs/release build gates, then signed+DCO commit and independent diff review. |
