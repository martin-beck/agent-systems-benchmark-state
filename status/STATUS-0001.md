...
| --- | --- | --- | --- | --- |
| P0 | [AR-1293](../tasks/AR-1293-state-tla-admission-runner.md): State-scoped TLA admission runner | ar1293_gate_recovery | Repair the state-repository TLA admission runner and truthful worktree metadata. | Approved VM console path is fixed, but pr-publication now fails at JVM startup with native memory allocation error because host swap is exhausted (100 KiB free) and concurrent VM/worker load remains active. Rerun pr-publication/full-exhaustive only on a clean runner with available swap and the known-good guest JVM; do not alter unrelated workloads. |
