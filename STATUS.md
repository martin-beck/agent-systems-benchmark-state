...
| --- | --- | --- | --- | --- |
| P0 | [AR-1060](tasks/AR-1060-authenticated-control-endpoint-handoff.md): Add authenticated control endpoint handoff | codex-ar1060-asb-provider-20260913-router | Hand the standalone frontend an authenticated ASB control connection without exposing endpoint paths. | Obtain immutable review of the frozen SOCK_SEQPACKET framing, then implement only the ASB authenticated-generation producer and broker state machine; keep launch and consumer behavior in AR-1024/1025. |
...
| P0 | [AR-1062](tasks/AR-1062-diagnose-nested-tmux-server-connect.md): Diagnose nested tmux server connection failures | Unclaimed | Diagnose the exact nested tmux server and rejected socket-error class on trusted main. | Re-run the exact Rust 1.93.0 full gates with corrected coverage invocation (&#96;bash ./tools/run-coverage-clean.sh&#96;). If clean, freeze and submit a signed DCO commit for immutable review. |
