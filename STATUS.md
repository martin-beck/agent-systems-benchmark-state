...
| --- | --- | --- | --- | --- |
| P1 | [AR-0313](tasks/AR-0313-all-agents-provider.md): Configure one provider for all agents | quality_20260906 | Let test plans select one provider profile for every chosen supported agent atomically. | Extend the green two-file atomic preflight core into the versioned provider-plan/config and CLI reporting surface without touching crates/asb-cli/src/control.rs; add stale/mixed/partial serialization and recovery negatives, then run full applicable gates. |
