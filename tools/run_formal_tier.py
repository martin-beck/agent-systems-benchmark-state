...
# Keep runtime state on the approved second disk even when the source checkout
# is a disposable hosted-runner path such as /home/runner/work.
APPROVED_RUNTIME_ROOT = Path("/srv/data/projects")
RUNTIME_ROOT = APPROVED_RUNTIME_ROOT / ".asb-tlc"
...
def environment(tier: str, *, runtime_root: Path = RUNTIME_ROOT) -> dict[str, str]:
    ...
    if not resolved.is_relative_to(APPROVED_RUNTIME_ROOT):
...
def _validate_runtime_environment(env: Mapping[str, str]) -> Path:
    """Validate approved runtime paths before starting the child process."""
    runtime_root = Path(env["TLC_RUNTIME_ROOT"]).resolve()
    if not runtime_root.is_relative_to(APPROVED_RUNTIME_ROOT):
        raise ValueError("runtime root must remain under /srv/data/projects")
    if Path(env["TMPDIR"]).resolve() != runtime_root:
        raise ValueError("TMPDIR must match the approved runtime root")
    if not Path(env["TLC_ATTESTATION_PATH"]).resolve().is_relative_to(APPROVED_RUNTIME_ROOT):
        raise ValueError("attestation path must remain under /srv/data/projects")
    _private_directory(runtime_root)
    return runtime_root


def _wait_for_process(tier: str, process: subprocess.Popen[bytes], timeout: int) -> int:
    """Wait for one child and classify bounded termination."""
    try:
...
def run(tier: str, *, environ: Mapping[str, str] | None = None) -> int:
    ...
    if environ is not None:
        env.update(environ)
    _validate_runtime_environment(env)
    command = [
    ...
    return _wait_for_process(tier, process, timeout)
